...
if(a._getDefinitions(a.schema),a.refs)for(var b in a.refs)a._getDefinitions(a.refs[b]);
// Validator options
var c={};a.options.custom_validators&&(c.custom_validators=a.options.custom_validators);
//@nlac: removed
//self.validator = new JSONEditor.Validator(self,null,validator_options);
// Create the root editor
var d=a.getEditorClass(a.schema);a.root=a.createEditor(d,{jsoneditor:a,schema:a.schema,required:!0,container:a.root_container}),
//@nlac: passing self.root.schema instead of null (to be able to use the resolved 'extend's)
a.validator=new f.Validator(a,a.root.schema,c),a.root.preBuild(),a.root.build(),a.root.postBuild(),
// Starting data
...
if(!b)return this.watchlist[a]=null,this;for(var c=[],d=0;d<this.watchlist[a].length;d++)this.watchlist[a][d]!==b&&c.push(this.watchlist[a][d]);return this.watchlist[a]=c.length?c:null,this},notifyWatchers:function(a){if(!this.watchlist||!this.watchlist[a])return this;for(var b=0;b<this.watchlist[a].length;b++)this.watchlist[a][b]()},isEnabled:function(){return!this.root||this.root.isEnabled()},enable:function(){this.root.enable()},disable:function(){this.root.disable()},_getDefinitions:function(a,b){if(b=b||"#/definitions/",a.definitions)for(var c in a.definitions)a.definitions.hasOwnProperty(c)&&(this.refs[b+c]=a.definitions[c],a.definitions[c].definitions&&this._getDefinitions(a.definitions[c],b+c+"/definitions/"))},_getExternalRefs:function(a){var b={},c=function(a){for(var c in a)a.hasOwnProperty(c)&&(b[c]=!0)};a.$ref&&"object"!=typeof a.$ref&&"#"!==a.$ref.substr(0,1)&&!this.refs[a.$ref]&&(b[a.$ref]=!0);
//@nlac:
//this._getDefinitions(schema);
for(var d in a)if(a.hasOwnProperty(d))if(a[d]&&"object"==typeof a[d]&&Array.isArray(a[d]))for(var e=0;e<a[d].length;e++)"object"==typeof a[d][e]&&c(this._getExternalRefs(a[d][e]));else a[d]&&"object"==typeof a[d]&&c(this._getExternalRefs(a[d]));return b},_loadExternalRefs:function(a,b){var c=this,e=this._getExternalRefs(a),f=0,g=0,h=!1;d(e,function(a){if(!c.refs[a]){if(!c.options.ajax)throw"Must set ajax option to true to load external ref "+a;c.refs[a]="loading",g++;var d=new XMLHttpRequest;d.open("GET",c.options.transformUrls?c.options.transformUrls(a):a,!0),//@nlac: added transformUrls
...
"undefined"!=typeof a[d]?(c.addObjectProperty(d),e.setValue(a[d],b)):b||c.isRequired(e)?e.setValue(e.getDefault(),b):(!c.schema.defaultProperties||c.schema.defaultProperties.indexOf(d)<0)&&//@nlac: added, to make defaultProperties work as it supposed
c.removeObjectProperty(d)}),d(a,function(a,d){c.cached_editors[a]||(c.addObjectProperty(a),c.editors[a]&&c.editors[a].setValue(d,b))}),this.refreshValue(),this.layoutEditors(),this.onChange()},showValidationErrors:function(a){var b=this,c=[],e=[];
...
c.on("change",function(){b.trigger("change")}),c.on("ready",function(){b.trigger("ready")}))}return this}}}(),window.JDornJSONEditor=f}();
